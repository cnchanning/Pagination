...
var Pagination = Widget.extend({
  ...
  setup: function() {
    ...
    this._initPanle();
    this.set("total_page", this._getNumPages(this.get("total_items"),
      this.get("items_per_page")));
    this._drawLinks();
  ...
  _getNumIntervel: function(total) {
    ...
    var start = this.get("current_page") > half ? Math.max(Math.min(+this
      .get("current_page") - half, limit), 1) : 1;
    var end = this.get("current_page") > half ? Math.min(+this.get(
      "current_page") + half - 2, total) : Math.min(+this.get(
      "num_display"), total);
    return [start, end];
  },
  /**
  ...
  _drawLinks: function() {
    ...
    var appendItem = function(page_id, appendopts) {
      var lnk;
      page_id = page_id + 1 < 1 ? 1 : (page_id < total ? page_id :
        total);
      ...
      if (page_id === current_page) {
        lnk = $("<span class=" + that.CLASS.UI_CURRENT + ">" + (
          appendopts.text) + "</span>");
      } else {
        lnk = $("<a href='javascript:void(0)'>" + (appendopts.text) +
          "</a>");
      }
    ...
    if (current_page && current_page > 1 || prev_always) {
      appendItem(current_page - 2, {
        text: prev_text,
        classes: this.CLASS.UI_PREV
      });
    }
    ...
    if (next_text && (current_page < total - 1 || next_always)) {
      appendItem(current_page + 3, {
        text: next_text,
        classes: this.CLASS.UI_NEXT
      });
    }
  ...
  _clickHandle: function(eve) {
    var target = $(eve.currentTarget).text();
    this.jumpTo(target);
  },
   ...
   */
  next: function() {
    var index = this.get("current_page") === this.get("total_page") ?
      this.get("total_page") : +this.get("current_page") + 1;
    this.jumpTo(index);
  },
  /**
   *向前一页
   */
  prev: function() {
    var index = this.get("current_page") === 1 ? 1 : +this.get(
      "current_page") - 1;
    this.jumpTo(index);
  },
  /**
